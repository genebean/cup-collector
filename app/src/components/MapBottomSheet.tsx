...
export function MapBottomSheet({ cups }: Props) {
  ...
  return (
    ...
    >
      {/* Handle / summary row */}
      <button
        className="w-full flex flex-col items-center bg-white rounded-t-2xl shadow-[0_-4px_12px_rgba(0,0,0,0.2)] pt-2 pb-2"
      ...
      {/* Scrollable cup list */}
      <div className="bg-white max-h-60 overflow-y-auto divide-y divide-gray-100">
        {cups.length === 0 ? (
          ...
          cups.map((cup) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium truncate">{cup.city}</div>
              ...
              <span
                className={`text-xs font-medium flex-shrink-0 ${
                  cup.isOwned ? "text-green-700" : "text-orange-600"
                }`}
              >
